document.addEventListener("DOMContentLoaded", () => {
  ...
  const artistNameLiked = document.getElementById("nomeartista");
  const artistImageLiked = document.getElementById("artist-image2");
  ...
  fetch(apiUrl)
    ...
    .then((data) => {
      ...
      // Aggiornamento dinamico della sezione "Brani che ti piacciono"
      artistNameLiked.textContent = `di ${data.name}`;
      artistImageLiked.src = data.picture_big;
    })
    .catch((error) => {
      hideLoader();
